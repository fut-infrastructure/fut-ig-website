# Artifacts Summary - eHealth Infrastructure v10.0.2
...
| [Create a careplan](OperationDefinition-PlanDefinition-i-apply.md) | Create a careplan with service requests for a specific episodeOfCare using the PlanDefinition as a template.### HTTP Methods* If using POST the operation will persist resources and return the careplan.
...
* Whenever an EpisodeOfCare is created, the server automatically creates an 'origin' Provenance resource point to it (in addition to the client-supplied Privacy-Provenance). This Origin-Provenance resource is filled with data from the JWT access token in order to record the origin of the EpisodeOfCare resource. For instance agent.whoReference points to a resource which is created as a shadow identity from the JWT token.
 |
| [Data migration](OperationDefinition--s-migrate.md) | Data migration |
...
Each resource is returned in a separate JSon file.### Parameters* Output can be found in Binary.content. This is a Base64 encoded .zip file containing JSon files. All returned objects are Fhir resources. See the implementation guide for details
 |
| [FHIR Bulk Data System Level Export](OperationDefinition--s-export.md) | FHIR Operation to initiate an export of data from a FHIR server. The type of resources returned can be restricted using the '_type' parameter, and the '_since' parameter can be used to restrict resources to be only included if changed since the supplied time. The FHIR server support invocation of this operation using the [FHIR Asynchronous Request Pattern](http://hl7.org/fhir/R4/async.html).## Initiation of exportThe initiation of an export must specify the request header 'Prefer: respond-async', and will return '202 Accepted' with a polling location in the response header 'Content-Location'. The polling location is for the endpoint '$export-poll-status' and has the export job ID as parameter '_jobId'.### Parameters* _outputFormat: Optional parameter to specify the output format of the exported files. Default is 'application/fhir+ndjson'
...
| [Retrieve count of assignments for material](OperationDefinition--s-assigned-material-count.md) | This operation counts the number of material assignments (`ehealth-material-communication resources`) for a given `ehealth-documentreference` from the Plan service. This is done by performing an internal search using the given ehealth-documentReference, date and status parameters. Regardless of the given parameters, only `ehealth-material-communication` resources referring to an active `ehealth-episodeofcare` are counted. The group-by-careteam parameter determines whether a simple '_summary=count' search is performed or if all results should be fetched to extract and count based on participant CareTeam references. ## Input The operation has two mandatory input parameters: * An EHealthDocumentReference reference 
* One or more status codes Additionally, there are two optional input parameters: 
...
* A mandatory reference to an EHealthDocumentReference (general-material) in the plan service. Only `ehealth-material-communication` resources with the given reference in `Communication.payload.content` are counted. 
  ...
  If group-by-careTeam was false or not present. The Parameters resource will only contain a single parameter, named `total` with the value being the count of the `ehealth-material-communication` resources found to satisfy the given parameters. Example of output with group-by-careTeam as false or not present: 
 |
...
| [expunge](OperationDefinition--s-expunge.md) |  |
| [expunge](OperationDefinition-Binary-its-expunge.md) |  |
| [hapi.fhir.reindex-status](OperationDefinition--s-hapi.fhir.reindex-status.md) |  |
| [import](OperationDefinition--s-import.md) | Import terminology resources |
| [meta](OperationDefinition--s-meta.md) |  |
| [reindex](OperationDefinition--s-reindex.md) |  |
| [reindex-terminology](OperationDefinition--s-reindex-terminology.md) |  |
...
| [careCommunicationSenderCareTeam](SearchParameter-ehealth-communication-search-careCommunicationSenderCareTeam.md) | Search parameter for finding CareCommunication based on the sending CareTeam (the careteam sub-extension of the ehealth-carecommunication-sender extension). |
| [careCommunicationSenderPractitioner](SearchParameter-ehealth-communication-search-careCommunicationSenderPractitioner.md) | Search parameter for finding CareCommunication based on the sending Practitioner (the practitioner sub-extension of the ehealth-carecommunication-sender extension). |
...
| [topic](SearchParameter-ehealth-communication-search-topic.md) | Search parameter for finding communication based on topic |
...
| |
| :--- |
| [ehealth-access-consent](StructureDefinition-ehealth-access-consent.md) |
...
| [Aggregate Group ID](StructureDefinition-ehealth-aggregate-group-id.md) | Id that link a group of measurements together by an aggregate group id, intended use is for Aggregated Triage. |
| [Aggregation Mode](StructureDefinition-ehealth-aggregation-mode.md) | Mode of Aggregated Triage |
...
| [CarePlan reference](StructureDefinition-ehealth-reference-careplan.md) | CarePlan reference |
...
| [Code Qualification](StructureDefinition-ehealth-code-qualification.md) | Condition qualification |
...
| [DateTime Extension](StructureDefinition-ehealth-carecommunication-datetime.md) | Date and time of the payload segment. |
...
| [Destination Extension](StructureDefinition-ehealth-carecommunication-destination.md) | Reference to the destination Organization for this communication. |
| [Device platform](StructureDefinition-ehealth-device-platform.md) | Platform identifier for the patient's mobile device (e.g. iOS, Android). Used together with the registration token to route push notifications via FCM/APNs. |
...
| [Ehealth provider affiliation](StructureDefinition-ehealth-provider-affiliation.md) | The Practitioner's association with a provider identifier (Danish: Ydernummer). |
...
| [Identifier Extension](StructureDefinition-ehealth-carecommunication-payload-identifier.md) | Extension to hold an Identifier for a payload. Identifier.value shall be a UUID v4 in URN form (urn:uuid:<uuid>). |
...
| [Managing organization](StructureDefinition-ehealth-managing-organization.md) | An organization that has assumed responsibility for the episode of care during a given period. Replaces the standard EpisodeOfCare.managingOrganization to allow multiple managing organizations over time. |
...
| [Message type](StructureDefinition-ehealth-carecommunication-message-Type.md) | The type of the message. inResponseTo is only allowed when the type is reply-message. |
...
| [Other item](StructureDefinition-ehealth-clinicalimpression-otherItem.md) | Investigation item for Aggregated Triage resources so they can be approved. |
...
| [Provider identifier](StructureDefinition-ehealth-organization-providerIdentifier.md) | The providerIdentifier of the organization (Danish: Tilknytning til ydernummer) |
...
| [Registration token](StructureDefinition-ehealth-device-registrationToken.md) | Registration token from a push notification service (e.g. FCM/APNs) for the patient's mobile device. Used by telemedicine solutions when sending push notifications. |
...
| [Sender Extension, contains the sending PractitionerRole, Practitioner and CareTeam.](StructureDefinition-ehealth-carecommunication-sender.md) | References the sending PractitionerRole (Actor), the Practitioner, and optionally a CareTeam. |
...
| [Telecom purpose](StructureDefinition-ehealth-telecom-purpose.md) | The purpose of a telecom contact point on a RelatedPerson. |
...
| [What the consent is related to](StructureDefinition-ehealth-consent-affiliation.md) | Which resources the consent is affiliated with, for instance EpisodeOfCare and CarePlan |
| [aggregateId](StructureDefinition-ehealth-aggregate-id.md) | An optional identifier used to associate this ServiceRequest with an aggregate or grouping context. |
...
| [sender organization](StructureDefinition-ehealth-carecommunication-origin.md) | Reference to the sending organization for this payload segment. |
...
| | |
| :--- | :--- |
| [AccessProvenance Activity](ValueSet-access-provenance-activity.md) | AccessProvenance Activity |
| [AccessProvenance Agent Role](ValueSet-access-provenance-agent-role.md) | AccessProvenance Agent Role |
...
| [Aggregation Mode Types](ValueSet-aggregation-mode-types.md) | Aggregation Mode |
| [All conditions (treatment area)](ValueSet-ehealth-treatment-area-xa-1.md) | Treatment area for all conditions (xa). |
| [Angst (treatment area)](ValueSet-ehealth-treatment-area-xc-2.md) | Treatment area for angst conditions (xc). |
...
| [Cardiovascular diseases (treatment area)](ValueSet-ehealth-treatment-area-xb-3.md) | Treatment area for cardiovascular diseases according (xb). |
...
| [Condition Qualification](ValueSet-vs-condition-qualification.md) | Value set of condition qualifications |
| [Condition Qualification for Tele Wound](ValueSet-condition-qualification-tele-wound.md) | Value set of condition qualifications for tele wound domain. |
...
| [Consent Policy](ValueSet-vs-ehealth-consent-policy.md) | Value set of policy for consent. |
| [Consent Provision Class](ValueSet-vs-ehealth-consent-provision-class.md) | Value set of provision class for consent. |
| [Consent Provision Code](ValueSet-vs-ehealth-consent-provision-code.md) | Value set of provision codes for consent. |
| [Consent Scope](ValueSet-vs-ehealth-consent-scope.md) | Value set of scope for consent. |
...
| [Depression (treatment area)](ValueSet-ehealth-treatment-area-xc-1.md) | Treatment area for depression conditions (xc). |
...
| [Device Platform](ValueSet-ehealth-device-platform.md) | Device Platform value set. |
...
| [Endpoint Connection Type Code](ValueSet-ehealth-endpoint-connection-type-code.md) | ValueSet of endpoint connection types. |
| [Endpoint Payload Type Code](ValueSet-ehealth-endpoint-payload-type-code.md) | ValueSet of endpoint payload types. |
...
| [Functional ability & social circumstances (treatment area)](ValueSet-ehealth-treatment-area-xb-6.md) | Treatment are for functional ability & social circumstances (xb). |
...
| [Mental disorders and mental health problems (treatment area)](ValueSet-ehealth-treatment-area-xb-1.md) | Treatment area for mental disorders and mental health problems (xb). |
| [Message Type ValueSet](ValueSet-MessageType.md) | Allowed message types: new-message, reply-message, forward-message. |
...
| [Neurological diseases (treatment area)](ValueSet-ehealth-treatment-area-xb-2.md) | Treatment area for neurological diseases (xb). |
| [OIO BPP Competences](ValueSet-ehealth-oio-bpp-competences.md) | OIO BPP competences. |
...
| [Other (treatment area)](ValueSet-ehealth-treatment-area-xb-7.md) | Treatment area for other (xb). |
...
| [Practitioner Competences](ValueSet-ehealth-practitioner-competences.md) | Practitioner Competences |
| [Priority](ValueSet-ehealth-priority.md) | Practitioner roles |
| [Pulmonary diseases (treatment area)](ValueSet-ehealth-treatment-area-xb-4.md) | Treatment area for pulmonary diseases (xb). |
...
| [RelationshipTypes](ValueSet-relatedperson-relationshiptype.md) | A set of codes that can be used to indicate the relationship between a Patient and a Related Person used in Denmark. Including the specific permissions granted to the Related Person in the context of a Power of Attorney relationship. |
| [Required policies for provenances](ValueSet-ehealth-required-provenance-policies.md) | Required policies under which data can be provided. One of these policies must be provided in a provenance. |
...
| [Somatic / metabolic diseases (treatment area)](ValueSet-ehealth-treatment-area-xb-5.md) | Treatment area for somatic / metabolic diseases (xb). |
...
| [Treatment Area Collection xa](ValueSet-ehealth-treatment-areas-xa.md) | Combined treatment areas for coexistence tag xa. |
| [Treatment Area Collection xb](ValueSet-ehealth-treatment-areas-xb.md) | Combined treatment areas for coexistence tag xb. |
| [Treatment Area Collection xc](ValueSet-ehealth-treatment-areas-xc.md) | Combined treatment areas for coexistence tag xc. |
...
| [Usage Context Body site](ValueSet-vs-usage-context-body-site.md) | Usage Body site |
| [Usage Context Task Type](ValueSet-vs-usage-context-task-type.md) | Value set of task types |
| [Usage Context Workflow](ValueSet-vs-usage-context-workflow.md) | Value set of workflows |
| [Usage Context for Tele Wound Body Site](ValueSet-usage-context-body-site-tele-wound.md) | Value set of body site for tele wound domain. |
| [Usage Context for Tele Wound Task Type ](ValueSet-vs-usage-context-task-type-tele-wound.md) | Value set of task types for the Tele Wound domain. |
| [Usage Context for Tele Wound Workflow ](ValueSet-vs-usage-context-workflow-tele-wound.md) | Value set of workflows for Tele Wound |
...
| [Wound Conditions](ValueSet-ehealth-wound-conditions.md) | Wound conditions. |
| [Z-diagnoser (treatment area)](ValueSet-ehealth-treatment-area-xc-3.md) | Treatment area for Z-diagnosis conditions (xc). |
| [eHealth CareCommunication Attachment MIME Types](ValueSet-ehealth-carecommunication-mimetypes.md) | Allowed MIME types for attachments in eHealth CareCommunication messages. Mirrors MedCom's medcom-core-attachmentMimeTypes ValueSet. |
| [eHealth CareCommunication Categories](ValueSet-ehealth-carecommunication-category.md) | Categories used for CareCommunciation messages. |
| [eHealth CareCommunication Priorities](ValueSet-ehealth-carecommunication-priority.md) | Priorities used for CareCommunication messages. |
| [eHealth Designation Use](ValueSet-ehealth-designation-use.md) | Details of how a designation would be used. |
| [eHealth Message Channel](ValueSet-ehealth-message-channel.md) | ValueSet for message channel codes. |
...
| [eHealth System](ValueSet-ehealth-system.md) | Coexistence tags for telemedicine solutions. |
| [eHealth Telecom Purpose](ValueSet-ehealth-telecom-purpose.md) | ValueSet for telecom purpose codes. |
...
| | |
| :--- | :--- |
| [AccessProvenance Activity](CodeSystem-access-provenance-activity.md) | AccessProvenance Activity |
| [AccessProvenance Agent Role](CodeSystem-access-provenance-agent-role.md) | AccessProvenance Agent Role |
...
| [Aggregation Mode](CodeSystem-ehealth-aggregation-mode-types.md) | Aggregation Mode |
...
| [Consent Scope](CodeSystem-cs-ehealth-consent-scope.md) | Consent scope |
| [Consent State Codes](CodeSystem-hl7.org-fhir-consent-state-codes.md) | Indicates the state of the consent |
| [Consent policy](CodeSystem-ehealth-consent-policy.md) | Consent policy |
...
| [Device Platform](CodeSystem-ehealth-device-platform.md) | Push notification service identifier for a citizen's mobile device. Used together with a registration token to direct push notifications to the right push notification service. |
...
| [ICD-11 for Mortality and Morbidity Statistics](CodeSystem-ICD-11-MMS-2025-01.md) | ICD-11 for Mortality and Morbidity Statistics, FHIR representation based on ICD-11 content from https://icd.who.int/en/. |
| [ICD-11 for Mortality and Morbidity Statistics - Danish Supplement](CodeSystem-ehealth.sundhed.dk-ICD-11-MMS-supplement.md) | Danish language supplement for ICD-11 for Mortality and Morbidity Statistics - Surface Topography. Translations to Danish provided by the Danish National Health Data Authority (Danish: Sundhedsdatastyrelsen). |
...
| [Message Type CodeSystem](CodeSystem-MessageTypeCS.md) | Allowed codes for message type. |
...
| [OIO-BPP Competences](CodeSystem-ehealth-oio-bpp-competences.md) | OIO BPP Competences |
...
| [Power of Attorney Privilege](CodeSystem-ehealth-poa-privilege.md) | Vendor-specific power of attorney privilege codes used by Keycloak when creating RelatedPerson resources for PoA-authenticated citizens. The full set of codes is externally governed and not enumerated here. |
| [Practitioner Competences](CodeSystem-ehealth-practitioner-competences.md) | Practitioner Competences |
...
| [Wound types](CodeSystem-telewound-wound-types.md) | Wound types |
| [eHealth CareCommunication Category codes](CodeSystem-ehealth-carecommunication-category.md) | The set of CareCommunication category code. |
| [eHealth CareCommunication Priority codes](CodeSystem-ehealth-carecommunication-priority.md) | The set of CareCommunication priority code. |
| [eHealth Designation Use](CodeSystem-ehealth-designation-use.md) | Codes used to identify/describe designation use. |
| [eHealth Message Channel codes](CodeSystem-ehealth-message-channel.md) | Codes indicating the channel used to send a message. |
...
| [eHealth Telecom Purpose codes](CodeSystem-ehealth-telecom-purpose.md) | Codes indicating the purpose of a telecom contact point. |
...
| [OIO BPP competences to eHealth Practitioner competences role mapping](ConceptMap-oio-bpp-competences-to-practitioner-competences.md) |  |
...
| [eHealth to MedCom CareCommunication Category Map](ConceptMap-ehealth-to-medcom-carecommunication-category.md) |  |
| [eHealth to MedCom CareCommunication Priority Map](ConceptMap-ehealth-to-medcom-carecommunication-priority.md) |  |
...
| [Consent/1](Consent-1.md) |  |
| [Consent/23](Consent-23.md) |  |
...
| [Provenance/1](Provenance-1.md) |  |
| [RelatedPerson with video-appointment SMS reminder telecom](RelatedPerson-relatedperson-videosms.md) | Example RelatedPerson configured to receive an SMS reminder when a related video appointment is upcoming. The telecom slice carries the `ehealth-telecom-purpose` extension with code `video-appointment-reminder-sms`. |
